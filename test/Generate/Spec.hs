...
spec = do
    it "generates correct output for Main module" $ do
        let output = generate defaultConfig ["Bar", "Foo"]
    ...
    it "omits main when module name is not Main" $ do
        let output = generate defaultConfig { moduleName = "MySpec" } ["Foo"]
        output `shouldBe` unlines
            [ "{-# LINE 1 \"test/Spec.hs\" #-}"
    ...
    it "includes LINE pragma with original path" $ do
        let output = generate defaultConfig { originalPath = "some/path/Spec.hs" } ["Foo"]
